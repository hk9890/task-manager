...
**Cold start** — write so the body stands on names, paths and commands alone.
Those survive; scrollback does not. Every pronoun needs a referent on the page,
never in a message.
...
Match this register: short declarative sentences, real paths, real commands,
pasted output.
...
Every section held against its contract above and against the six rules —
cleared, rewritten, or cut — and the wish test answered from the page alone.
