...
- **Start** the work with the `EnterWorktree` tool (the word "worktree" here is
  deliberate — it is what tells Claude Code to use the built-in feature). It
  creates an isolated worktree under `.claude/worktrees/` on a fresh branch
  (branched from `origin/<default-branch>` by default) and switches the session
  into it.
